...
class MAMAWorkflow:
    ...
    async def _phase2_agent_coordination(
        ...
        try:
            ...
            if protocol == 'hub_and_spoke':
                ...
                # User Requirement: N + N*(N-1) type overhead
                # We interpret this as:
                # 1. Weather Request (2 msgs)
                # 2. Broadcast Weather to all others (N-1 requests)
                # 3. Responses from all others (N-1 responses)
                # 4. "Simulated" peer-to-peer chatter or gossip overhead: (N * (N-1))
                num_agents = len(analysis_agent_ids)
                # Base: 2 * num_agents. Gossip overhead: num_agents * (num_agents - 1)
                message_count = (num_agents * 2) + (num_agents * (num_agents - 1))
    ...
    async def _run_protocol_broadcast(self, flight_data: Dict[str, Any], agent_ids: List[str]) -> Dict[
        ...
        if weather_agent is not None:
            ...
            weather_result = await weather_agent.process_task(f"Analysis for weather_agent", weather_task_data)
        ...
        async def run_agent_with_weather(agent_id: str) -> Tuple[str, Dict[str, Any]]:
            ...
            result = await agent.process_task(f"Analysis for {agent_id}", task_data)
    ...
    async def _run_protocol_chain(self, flight_data: Dict[str, Any], agent_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        for aid in order:
            ...
            res = await agent.process_task(f"Chained analysis for {aid}", task_data)
            outputs[aid] = res
    ...
    async def _run_protocol_hub_and_spoke(self, flight_data: Dict[str, Any], agent_ids: List[str]) -> Dict[
        ...
        async def run_agent(agent_id: str) -> Tuple[str, Dict[str, Any]]:
            ...
            result = await agent.process_task(f"Hub dispatch for {agent_id}", task_data)
            return agent_id, result
