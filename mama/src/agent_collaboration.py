...
class WeatherAgent (BaseAgent ):
    ...
    async def process_task(self, task_description: str, task_data: Dict[str, Any]) -> Dict[str, Any]:
        # Artificial latency to simulate computation and highlight protocol differences
        # Chain (sequential) will accumulate this latency, while Hub/Broadcast (parallel) will not.
        import asyncio
...
class SafetyAssessmentAgent (BaseAgent ):
    ...
    def assess_flight_safety (self ,flight_data :Dict [str ,Any ],context :Optional [Dict [str ,Any ]]=None )->SafetyAssessment :
        ...
        for _ ,row in df .iterrows ():
            ...
            try :
                if weather_ctx and isinstance (weather_ctx ,dict )and weather_ctx .get ('status')=='success':
...
class FlightInfoAgent (BaseAgent ):
    ...
    def analyze_flight_information (self ,flight_data :Dict [str ,Any ])->FlightInfoAnalysis :
        ...
        for _ ,row in df .iterrows ():
            ...
            # Context gain from Weather (Broadcast/Chain)
            weather_ctx = context.get('weather_context')
            if weather_ctx and isinstance(weather_ctx, dict) and weather_ctx.get('status') == 'success':
                w_score = float(weather_ctx.get('safety_score') or 0.5)
                if w_score < 0.4:
                    operational_score = max(0.05, operational_score - 0.1)
                elif w_score > 0.8:
                    operational_score = min(0.99, operational_score + 0.05)
    ...
    def analyze_flight_economics (self ,flight_data :Dict [str ,Any ])->EconomicAnalysis :
        ...
        for _ ,row in df .iterrows ():
            ...
            if weather_ctx and isinstance(weather_ctx, dict) and weather_ctx.get('status') == 'success':
                w_score = float(weather_ctx.get('safety_score') or 0.5)
                if w_score < 0.4:
                    # Poor weather increases delay risk -> higher cost -> lower score
                    overall_score = max(0.05, overall_score - 0.15)
    ...
    trust_scores :Dict [str ,float ],flight_data :Dict [str ,Any ]=None )->IntegrationResult :
        ...
        if per_flight_df .empty :
            ...
            for cand in candidate_list :
                ...
                # Add small random noise to prevent perfect 1.0 MRR (simulate real-world uncertainty)
                integrated += random.uniform(-0.02, 0.02)
                integrated = max(0.01, min(0.99, integrated))
...
class MARLCollaborationEngine :
    ...
    async def execute_collaboration_task (self ,task :CollaborationTask )->CollaborationResult :
        ...
        try :
            ...
            integrated_result =await self ._integrate_agent_results (agent_results ,trust_scores ,task )
        ...
        async def execute_agent_task (agent_id :str )->Dict [str ,Any ]:
            if agent_id in self .agents :
                try :
                    ...
                    result =await agent .process_task (task_description ,task .flight_data )
    ...
    async def _integrate_agent_results (self ,agent_results :Dict [str ,Dict [str ,Any ]],
    ...
    task :CollaborationTask )->Dict [str ,Any ]:
        try :
            ...
            if integration_agent :
                ...
                integration_result =await integration_agent .process_task (
